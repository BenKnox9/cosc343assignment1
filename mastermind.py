...
class Player:
    def __init__(self, playerFile, code_length, colours, num_guesses):
        ...
        if not os.path.exists(playerFile):
            raise RuntimeError(
                "Error! Agent file '%s' not found" % self.playerFile)

        if len(playerFile) > 3 and playerFile[-3:].lower() == '.py':
            playerModule = playerFile[:-3]
        else:
            raise RuntimeError(
                "Error! Agent file %s needs a '.py' extension" % self.playerFile)
        ...
        try:
            self.agent = self.exec.MastermindAgent(
                code_length=code_length, colours=colours, num_guesses=num_guesses)
        except Exception as e:
            raise RuntimeError(str(e))
...
class MastermindGame:
            ...
            try:
                if not isinstance(actions, list) and not isinstance(actions, np.ndarray):
                    if actions == None:
                        ...
                        self.throwError("Error! AgentFunction from '%s.py' returned a %s (expecting a list or a numpy array)" % (
    ...
    def run(self, agentFile='agent_human.py', num_guesses=6, num_games=1000, seed=None):
        ...
        try:
            player = Player(playerFile=agentFile, code_length=self.code_length, colours=list(
                self.colours), num_guesses=num_guesses)
        except Exception as e:
            self.throwError(str(e))
        ...
        for i in I:
            ...
            start = time.time()
            score += self.play(player,
                               target=self.colours[i], num_guesses=num_guesses)
            end = time.time()
            game_count += 1
            print("Average score after game %d: %.2f" %
                  (game_count, score/(game_count)))
            tot_time += end - start

            if game_count < num_games:
                avg_time = tot_time / game_count
                print("Average running time per game %s." %
                      (time_to_str(avg_time)))
                print("Time remaining %s." %
                      (time_to_str(avg_time * (num_games-game_count))))
                print("Expected total running time %s." %
                      (time_to_str(avg_time * num_games)))
            else:
